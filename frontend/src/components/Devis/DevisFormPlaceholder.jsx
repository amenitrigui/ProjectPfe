...
const DevisFormPlaceholder = () => {
    ...
    const fetchClients = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${selectedDatabase}/clients`
        );
  ...
  useEffect(() => {
    const fetchFamilles = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${selectedDatabase}/familles`
        );
  ...
  const handleFamilleChange = async (index, famille) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${selectedDatabase}/codes/famille/${famille}`
      );
  ...
  const handleCodeArtChange = async (index, code) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${selectedDatabase}/articles/details/${code}`
      );
  ...
  const updateConfig = async (index, newConfigValue) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${selectedDatabase}/articles/${code}/updateConfig`,
    ...
    if (selectedOption) {
      ...
      if (selectedCode) {
        try {
          ...
          const response = await axios.get(
            `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${dbName}/clients/code/${selectedCode}`
          );
    ...
    if (selectedOption) {
      ...
      if (selectedRsoc) {
        try {
          ...
          const response = await axios.get(
            `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${dbName}/clients/rsoc/${selectedRsoc}`
          );
  ...
  useEffect(() => {
    const fetchLibpv = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${dbName}/devis/libpv/${selectedDevis}`
        );
  ...
  const fetchNumbl = async (datebl) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${selectedDatabase}/last-numbl?datebl=${formattedDate}`
      );
  ...
  const fetchDevisList = async (database) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/utilisateurs/get-devis-details/${database}`,
        {
          headers: {
  ...
  const fetchDevisDetails = async (database, NUMBL) => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/utilisateurs/get-devis-details/${database}/${NUMBL}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    const fetchRepresentants = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/get-representant-details/${dbName}/${formData}`
        );
  ...
  const removedevis = async () => {
    ...
    if (!numbl || !selectedDatabase) {
      setMessage({
        type: "error",
        text: "Aucun devis ou base de données sélectionnée pour suppression.",
    ...
    try {
      ...
      const response = await axios.delete(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${selectedDatabase}/devis/${numbl}`
      );

      if (
        response.status === 200 &&
        response.data.message.includes("Le devis")
      ) {
        setMessage({
          type: "success",
        ...
        localStorage.removeItem("selectedDevisNumbl");
  ...
  const validateNewMode = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${dbName}/create`,
        dataToSend
      );
  ...
  const handleUpdateDevis = async () => {
    try {
      ...
      const formattedLignes = allLignes.map((ligne) => ({
        code: ligne.CodeART || "",
        libelle: ligne.DesART || "",
        nbrunite: ligne.QteART || 0,
        prix1: ligne.PUART || 0,
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/devis/${database}/${formData.NUMBL}`,
        requestBody,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const fetchSecteurs = async () => {
    try { 
      const response = await fetch(
        `${process.env.REACT_APP_API_URL.replace(/\/$/, '')}/api/utilisateurs/secteurs/ERP13`
      );
      const data = await response.json();
  ...
  return (
    <div>
      <div>
        {selectedRsoc && selectedDatabase && (
          <div className="absolute top-4 left-0 w-full text-black text-sm font-semibold py-2 px-4 shadow-lg z-10">
            Gestion Commerciale...({selectedDatabase}, {selectedRsoc})
            utilisateur {user}
          </div>
        )}
      </div>
                  ...
                  <div>
                    ...
                    {isNewMode || formData.RSCLI === "" ? (
                      ...
                      <input
                        ...
                        className="w-full border border-gray-300 rounded-md p-2"
                        onChange={(e) =>
                          setFormData({ ...formData, RSCLI: e.target.value })
                        }
                      />
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {isNewMode ? (
                      ...
                      <input
                        ...
                        className="w-full border border-gray-300 rounded-md p-2"
                        onChange={(e) =>
                          setFormData({ ...formData, ADRCLI: e.target.value })
                        }
                      />
                    )}
                  </div>
                ...
                <div className="flex items-center space-x-2">
                  <div className="w-full">
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 rounded-md p-2"
                      onChange={(e) =>
                        setFormData({ ...formData, RSREP: e.target.value })
                      }
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  <div className="w-full">
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 rounded-md p-2"
                      onChange={(e) =>
                        setFormData({ ...formData, desisec: e.target.value })
                      }
                    />
                  </div>
                </div>
...
export default DevisFormPlaceholder;
